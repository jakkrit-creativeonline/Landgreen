...
class SubmanagerDashboard extends StatefulWidget {
  final int userId;

  const SubmanagerDashboard({Key key, this.userId}) : super(key: key);

  @override
  _SubmanagerDashboardState createState() => _SubmanagerDashboardState();
}

class _SubmanagerDashboardState extends State<SubmanagerDashboard> {
  int carCount = 0;
  int teamCount = 0;
  int salesCount = 0;
  var ds = new Map();
  ...
  //   var res =
  //       await Sqlite().getJson('SUBMANAGER_TEAM_SALE', '${widget.userId}');
  //   if (res != null) {
  ...
  //       Sqlite()
  //           .insertJson('SUBMANAGER_TEAM_SALE', '${widget.userId}', res.body);
  ...
  Future<Null> getAllBillDataThisMonth() async {
    ...
    List data = [];
    var res =
        await Sqlite().getJson('SUBMANAGER_ALL_BILL_DATA', '${widget.userId}');
    if (res != null) {
      ...
      if (isConnect) {
        ...
        Sqlite().insertJson(
            'SUBMANAGER_ALL_BILL_DATA', '${widget.userId}', res.body);
  ...
  Future<Null> getBillDataThisMonthForTeamSorkRank() async {
    teamDataRank = [];
    var res =
        await Sqlite().getJson('SUBMANAGER_TEAM_BILL_DATA', '${widget.userId}');
    if (res != null) {
      ...
      if (isConnect) {
        ...
        Sqlite().insertJson(
            'SUBMANAGER_TEAM_BILL_DATA', '${widget.userId}', res.body);
      }
    }
    teamData.sort((a, b) =>
    ...
    for (int i = 0; i < teamData.length; i++) {
      if (i < 5) {
        teamData[i]['rank'] = i + 1;
        teamDataRank.add(teamData[i]);
      } else {
        teamData[teamData.length - 1]['rank'] = teamData.length;
        teamDataRank.add(teamData[teamData.length - 1]);
        break;
      }
    }
    ...
    for (int i = 0; i < teamDataRank.length; i++) {
      if (i == 0) {
        chartDataRanking = {
          'rank ${i + 1}': SaleRanking(
              teamDataRank[i]['rank'],
              teamDataRank[i]['Head_sum_count_product_cat1'],
              teamDataRank[i]['Head_name'],
              charts.ColorUtil.fromDartColor(chartBarColor[i]),
              legendColor: chartBarColor[i],
              imgAvatar: teamDataRank[i]['Head_image'])
        };
      } else {
      ...
      return GestureDetector(
        onTap: () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  settings: RouteSettings(name: 'สีเหลืองดูข้อมูลภายใต้สายงาน'),
